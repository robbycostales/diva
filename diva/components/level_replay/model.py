...
from diva.components.level_replay.distributions import Categorical
from diva.components.level_replay.envs import PROCGEN_ENVS
from diva.components.level_replay.utils import init

init_ = lambda m: init(m, nn.init.orthogonal_, lambda x: nn.init.  # noqa: E731
                        constant_(x, 0))

init_relu_ = lambda m: init(m, nn.init.orthogonal_, lambda x: nn.init.  # noqa: E731
                        constant_(x, 0), nn.init.calculate_gain('relu'))

init_tanh_ = lambda m: init(m, nn.init.orthogonal_, lambda x: nn.init.  # noqa: E731
...
def model_for_env_name(args, env):
    if args.domain.env_name in PROCGEN_ENVS:
        model = Policy(
            env.observation_space.shape, env.action_space.n,
            arch=args.arch,
            base_kwargs={'recurrent': False, 'hidden_size': args.hidden_size})
    elif args.domain.env_name.startswith('MiniGrid'):
