...
class A2C:
    def __init__(self,
        ...
        if policy_anneal_lr:
            lam = lambda f: 1 - f / train_steps  # noqa: E731
            self.lr_scheduler_policy = optim.lr_scheduler.LambdaLR(self.optimiser, lr_lambda=lam)
            if hasattr(self.args, 'rlloss_through_encoder') and self.args.vae.rlloss_through_encoder:
    ...
    def update(self,
        ...
        if rlloss_through_encoder:
            ...
            utl.recompute_embeddings(policy_storage, encoder, sample=False, update_idx=0,
                                     detach_every=self.args.vae.tbptt_stepsize if hasattr(self.args,
        ...
        for sample in data_generator:
            ...
            latent_batch = utl.get_zs_for_policy(args=self.args, latent_sample=latent_sample_batch,
            ...
            if rlloss_through_encoder:
                loss += self.args.vae.vae_loss_coeff * compute_vae_loss()

            # compute gradients (will attach to all networks involved in this computation)
            loss.backward()
            nn.utils.clip_grad_norm_(self.actor_critic.parameters(), self.args.policy.max_grad_norm)
            if encoder is not None and rlloss_through_encoder:
                nn.utils.clip_grad_norm_(encoder.parameters(), self.args.policy.max_grad_norm)
        ...
        if (not rlloss_through_encoder) and (self.optimiser_vae is not None):
            for _ in range(self.args.vae.num_updates):
