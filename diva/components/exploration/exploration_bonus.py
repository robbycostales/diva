...
from diva.components.exploration.rnd_bonus import RNDRewardBonus
from diva.components.exploration.rollout_storage import RolloutStorage
from diva.environments import utils as utl
from diva.environments.utils import RunningMeanStd
...
class ExplorationBonus(object):
    def __init__(self,
        ...
        elif self.args.policy.pass_latent:
            self.dim_belief = self.args.vae.latent_dim * 2
        else:
            self.dim_belief = 0
        ...
        if self.args.exploration_bonus_hyperstate:
            ...
            self.ret_hs = torch.zeros(size=(self.args.policy.num_processes, 1)).to(DeviceConfig.DEVICE)
        if self.args.exploration_bonus_state:
            # for normalising the rew
            self.ret_s_rms = RunningMeanStd(shape=())
            # discounted return for each environment
            self.ret_s = torch.zeros(size=(self.args.policy.num_processes, 1)).to(DeviceConfig.DEVICE)
        if self.args.exploration_bonus_belief:
            # for normalising the rew
            self.ret_b_rms = RunningMeanStd(shape=())
            # discounted return for each environment
            self.ret_b = torch.zeros(size=(self.args.policy.num_processes, 1)).to(DeviceConfig.DEVICE)
        if self.args.exploration_bonus_vae_error:
            # for normalising the rew
            self.ret_v_rms = RunningMeanStd(shape=())
            # discounted return for each environment
            self.ret_v = torch.zeros(size=(self.args.policy.num_processes, 1)).to(DeviceConfig.DEVICE)
    ...
    def reward(self,
        ...
        with torch.no_grad():
            ...
            if self.args.exploration_bonus_vae_error:
                ...
                intrinsic_vae_bonus = self.args.vae.rew_loss_coeff * rew_reconstruction_loss + \
    ...
    def update(self, frames, iter_idx, log=True):
        ...
        if self.anneal_weights:
            self.curr_weight = 1 - frames / self.args.domain.num_frames
        ...
        elif self.args.policy.pass_belief:
            # get a batch of data
