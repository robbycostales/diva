...
class DictRolloutStorageVAE(object):
    ...
    def get_batch(self, batchsize=5, replace=False):
        ...
        rollout_indices = np.random.randint(0, self.buffer_len, size=batchsize)
...
class RolloutStorageVAE(object):
    def __init__(self, 
        ...
        if self.max_buffer_size > 0:
            ...
            if task_dim is not None:
                ...
                self.tasks = None
            # self.trajectory_lens = [0] * self.max_buffer_size
            self.trajectory_lens = np.zeros((self.max_buffer_size)).astype(int)
    ...
    def get_batch(self, batchsize=5, replace=False):
        ...
        to_return = (prev_obs.to(DeviceConfig.DEVICE), next_obs.to(DeviceConfig.DEVICE), actions.to(DeviceConfig.DEVICE), \
